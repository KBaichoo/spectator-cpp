...
struct PublisherConfig {
  ...
  size_t num_seperate_buffers = 16;  // default value of 16
};

class SpectatordPublisher {
 public:
  explicit SpectatordPublisher(
      absl::string_view endpoint, uint32_t bytes_to_buffer = 0,
      std::chrono::milliseconds flush_interval = std::chrono::milliseconds(60000),
      std::shared_ptr<spdlog::logger> logger = DefaultLogger());

  explicit SpectatordPublisher(absl::string_view endpoint, const PublisherConfig& config,
                               std::shared_ptr<spdlog::logger> logger = DefaultLogger());
 ...
 private:
  ...
  void local_reconnect(absl::string_view path, size_t socket_index);
  void udp_reconnect(const asio::ip::udp::endpoint& endpoint, size_t socket_index);

  void start_worker_threads();
  void stop_worker_threads();
  void worker_thread_loop(size_t worker_index);
  ...
  std::vector<std::unique_ptr<asio::ip::udp::socket>> udp_sockets_;
  std::vector<std::unique_ptr<asio::local::datagram_protocol::socket>> local_sockets_;
